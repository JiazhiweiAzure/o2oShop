...
<script type="text/javascript">
	function findother(){
		...
		if (yyy > yyyy) {
			...
			location.href = "poundage.do?uname=" + uname + "&yyy=" + yyy
	...
	<section id="main-content">
		<section class="wrapper">
			<div class="content-panel">
				<h3>
					&nbsp; &nbsp;<i class="fa fa-angle-right"></i> <a href="poundage.do">手续费记录</a>
				</h3>
				<br> &nbsp; &nbsp; &nbsp; 
				...
				<div class="row clearfix">
					<div class="col-md-12 column">
						<table class="table table-striped table-hover">
							...
							<tbody>
								<c:forEach items="${lpa }" var="w" varStatus="i">
									<tr>
										<td>${i.index+1}</td>
										<td>${w.uname }</td>
										<td>${w.zname }</td>
										<td>${w.sxmoney }</td>
										<td>${w.what }</td>
										<td><fmt:formatDate value="${w.sxtime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
									</tr>
								</c:forEach>	
							</tbody>			
					</table>
				...
				&nbsp; &nbsp;&nbsp;	<ul class="pagination">
				<li>
					 <a href="poundage.do?currpage=1&uname=${uname }&zname=${zname}&yyy=${yyy}&yyyy=${yyyy}">首页</a>
				</li>
				<li>
					 <a href="poundage.do?currpage=${currpages-1 }&uname=${uname }&zname=${zname}&yyy=${yyy}&yyyy=${yyyy}">上一页</a>
				</li>
				<c:forEach begin="1" end="${totalpage }" varStatus="i">
								<li><a
									href="poundage.do?currpage=${i.index}&uname=${uname }&zname=${zname}&yyy=${yyy}&yyyy=${yyyy}">${i.index}</a>
								</li>
							</c:forEach>
				<li>
					 <a href="poundage.do?currpage=${currpages+1}&uname=${uname }&zname=${zname}&yyy=${yyy}&yyyy=${yyyy}">下一页</a>
				</li>
				<li>
					 <a href="poundage.do?currpage=${totalpage}&uname=${uname }&zname=${zname}&yyy=${yyy}&yyyy=${yyyy}">尾页</a>
				</li>
			</ul>
					</div>
				</div>
